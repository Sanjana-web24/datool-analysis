...
# Function to get all Excel files from a local directory
def get_local_excel_files(directory):
    file_links = []
    for filename in os.listdir(directory):
        if filename.endswith('.xlsx'):
            file_links.append(os.path.join(directory, filename))
    return file_links
...
# Main execution
if __name__ == "__main__":
    choice = input("Do you want to fetch files from SVN (type 'svn') or from a local folder (type 'local')? ").strip().lower()
    
    if choice == 'svn':
        excel_files = get_excel_file_links(svn_link)
    elif choice == 'local':
        local_directory = input("Please enter the local directory path: ").strip()
        excel_files = get_local_excel_files(local_directory)
    else:
        print("Invalid choice. Please type 'svn' or 'local'.")
        excel_files = []
